...
/**
 */
public class OverlapResolveStrengthNoMrg implements UnaryGraphToGraphOperator {
    private Double delta;
    public OverlapResolveStrengthNoMrg (Double inputDelta){delta = inputDelta;}
    @Override
...
//
//        // process overlapped vertices
//        vertex_oldClsId_newClsId = toBeResolvedVertexId_vertex_cluster.groupBy(0).reduceGroup(new NoMergeResolver(ResolveIteration.ITERATION2));
//        DataSet<Tuple2<Vertex, String>> updatedVertices = vertex_oldClsId_newClsId.map(new getVertex_VId()).distinct(1);
//
//
//        // update vertices
//        DataSet<Tuple2<Vertex, String>> allVertices = vertices.map(new vertex2vertex_gradoopId());
//        vertices = new minus().execute(allVertices, updatedVertices);
