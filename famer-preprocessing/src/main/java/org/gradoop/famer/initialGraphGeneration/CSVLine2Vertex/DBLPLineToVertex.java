 ...
 */
public final class DBLPLineToVertex
        implements FlatMapFunction<String , Vertex> {
    private VertexFactory vf;
    public DBLPLineToVertex (VertexFactory VertexFactory){
        vf = VertexFactory;
    }
    ...
    public void flatMap(String in, Collector<Vertex> out) throws Exception {
        ...
        if (in.indexOf("\"") < 0||(in.contains("id") && in.contains("author") && in.contains("title")))
            firstLine = true;
        if (!firstLine) {
            while (pairs <= 3) {
